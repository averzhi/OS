...
#include <unistd.h> // позволяет получить доступ к функциям (API) операционных систем POSIX 
#include <string>	// библиотека работы с форматом String
#include <sstream>  // взаимодействие со строковым типом - позволяет связать поток ввода-вывода со строкой в памяти
#include <exception>// обработка исключений 
#include <signal.h>
#include "zmq.hpp" // ZeroMQ (обмен сообщениями, работа с очередями)

#include "timer.h" // наша библиотека таймера

const int WAIT_TIME = 1000;  // время ожидания (чего-то)
const int PORT_BASE  = 5050; // базовый порт (???)
...
/*
	@description: функция получения сообщений
*/
...
/*
	@description: функция создания ноды (???)
*/
...
/*
	@description: функция получения имени порта
*/
...
/*
	@description: функция выполнения чего-либо из доступного списка
	варианты: 
	start - 
	stop - 
	time - 
...
/*
	@description: функция уничтожения дочернего процесса
*/
